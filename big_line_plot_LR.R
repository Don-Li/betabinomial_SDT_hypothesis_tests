make_big_line_plot2 = function( this_prime, dataset, legend, criterion ){
    text_labels = dataset[ d_prime == this_prime & n_trials %in% c(8, 32, 256)][ overdispersion == 0.1, {
        list( x = overdispersion, y = 1-bbinom_LR_CI_prop_given_binom, label = n_replications )
    }, by = c("n_trials") ]
    ...
    big_lines = ggplot() +
        geom_point( aes( x = overdispersion, y = 1-bbinom_LR_CI_prop_given_binom ), dataset,
            size = 0.5 ) +
        geom_line( aes( x = overdispersion, y = 1-bbinom_LR_CI_prop_given_binom, 
